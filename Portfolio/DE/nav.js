document.addEventListener("DOMContentLoaded", () => {
  ...
  burgerMenu.addEventListener('click', function(event) {
    navigationContainer.style.display = navigationContainer.style.display === 'block' ? 'none' : 'block';
    event.stopPropagation();
  });

  document.addEventListener('click', function(event) {
    
    if (navigationContainer.style.display === 'block' && !navigationContainer.contains(event.target) && !burgerMenu.contains(event.target)) {
      navigationContainer.style.display = 'none';
    }
  });

  // Aquí puedes agregar la llamada a cualquier otra función que necesites ejecutar.
});
